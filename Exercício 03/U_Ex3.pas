...
begin
  ...
  listPilha.Clear;
  PilhaCopia := TPilha.Create; // Cria uma cópia da pilha
  // Preenche a cópia da pilha e exibe os itens no ListBox da pilha
  while Pilha.Count > 0 do
  begin
    Pessoa := Pilha.Unstack;
    listPilha.Items.Add(Pessoa.nome);
    PilhaCopia.StackUp(Pessoa); // Armazena a cópia na ordem correta
  end;
  try
    Queue := TQueue.Create;
    lbFila.Clear;
